...
- Ejecutadas las primeras 4 olas del plan `docs/auditoria-arquitectura/08-plan-modularizacion.md` (detalle completo y decisiones AHÍ — este resumen es el índice):
  ...
  - **Ola 3**: islas `dev-cdp.html`, `formacion.html`, `politicas.html`, `impacto.html` (con `eqp*`) + **`core-isla.js`** compartido versionado (esc, islaFetch/Bearer, islaUser, tema+storage event, toast, helpers ready/vista/ruta/badge/pedir-tarea). `empaque` difirió a propósito: será la 1ª isla con upload de fotos, PR propio (su `apiFetch` quedó restaurado en el shell).
