...
## Routing por URL — hash router (v224–v226, 22-jul-2026)
- La SPA ahora es direccionable: `#<seccion>` (Despachos) y `#wwp/<tab>` (Workforce). Deep-links, F5 y atrás/adelante del navegador funcionan; el aterrizaje post-auth honra el hash si `canSection`/`hasWwpAccess` lo permiten (silencioso si no). Regla clave: **subruta vacía = default de la sección** (la URL base y el default son la misma ruta → sin doble push al entrar).
