...
- **2026-06-13 · Sidebar: wordmark tipográfico en vez de imagen logo · Gabriel pidió reemplazar `descarga.png` (54×54px, cuadrado gris con "ALTRI TEMPI" serif) que ocupaba ~76px en el header del sidebar y no armonizaba con el navy. Solución: `.sidebar-brand` con wordmark "ALTRI TEMPI" en Georgia serif blanco 13px + subtítulo "Dashboard Operativo" en muted uppercase 9px + dots de conexión (Sheets/Odoo) integrados inline a la derecha en la misma fila. Eliminado `.sidebar-conn-bar` como fila separada. Resultado: header de ~44px vs 76px anterior, sin bloque gris disruptivo, armoniza con el navy del sidebar. Regla: en sidebars oscuros, el wordmark tipográfico puro es más premium que una imagen cuadrada — imagen solo si tiene fondo transparente y es apropiada para escala pequeña.**

- **2026-06-13 · Fix sticky nav tabs: `top:44px → 49px` · La barra `.app-nav` (Tareas/Dashboard/Usuarios) se movía unos píxeles al hacer scroll antes de hacer sticky. Causa: `top:44px` no coincidía con la altura real del `.app-topbar`. Cálculo: `.notif-btn` tiene `height:36px` (el elemento más alto del topbar) + `padding:6px 24px` (6+6=12px vertical) + `border-bottom:1px` = 49px total. Con `top:44px` la nav se sumergía 5px bajo el topbar antes de anclar. Fix: `top:49px`. Regla permanente: cuando `.app-nav` tiene `position:sticky`, el valor de `top` debe ser la altura exacta del elemento sticky que le precede (topbar). Calcular como: altura del elemento más alto dentro del topbar + padding vertical + border. No usar valor estimado.**

- **2026-06-13 · Sidebar activo: `setNavActive('wwp')` tras cambiar a screen-app · Al hacer WWP la pantalla default, `setHistorialUser()` llamaba `navTo('buscar')` internamente para no-admin, lo que marcaba "Buscador" como activo en el sidebar aunque la pantalla visible fuera WWP. Fix: agregar `setNavActive('wwp')` en todos los puntos de routing donde se muestra `screen-app` como destino. Regla: siempre que se navegue programáticamente a una pantalla, llamar `setNavActive()` al final para garantizar que el sidebar refleje la pantalla real visible — no el último `navTo()` que corrió en background.**

- **2026-06-13 · WWP como pantalla de aterrizaje por defecto (todos los usuarios) · Gabriel pidió que al entrar o refrescar, todos los usuarios — incluidos admin y roles con secciones Despachos — aterricen en Workforce Platform (`screen-app`) en vez de Historial/Despachos (`screen-historial`). Cambio en 5 puntos de routing (`checkLogin`, `doRefresh`, `doLogin`, impersonate, stop-impersonate): en todos se llama ahora `if(isHistorialUser) setHistorialUser()` + siempre `enterApp(); showScreen('screen-app'); _wwpEntered=true`. `setHistorialUser()` sigue ejecutándose para aplicar permisos de nav del sidebar de Despachos (oculta secciones sin acceso), pero la pantalla visible al abrir es siempre WWP. El sidebar de Despachos sigue accesible para navegar a historial. No se rompe la función `goToHistorial()` ni el toggle de shells. Verificado: grep confirma 0 ocurrencias de `setHistorialUser(); showScreen('screen-historial')` en el código.**
...
- **En sidebars oscuros, el wordmark tipográfico es más premium que una imagen cuadrada.** Logo tipo imagen solo si tiene fondo transparente y escala bien a tamaños pequeños. Un bloque cuadrado gris en un sidebar navy rompe la armonía cromática. Georgia serif blanco + subtítulo muted funciona perfectamente como identidad de marca en sidebar oscuro. 🌐
- **El valor `top` de un elemento sticky siempre debe ser la altura exacta del elemento sticky que le precede.** Calcularlo como: max-height del contenido + padding vertical + border. Nunca usar valor estimado. Si el elemento superior tiene `padding:6px` vertical y contenido de 36px + 1px border = 49px. 🌐
- **Después de navegar programáticamente a una pantalla, siempre llamar `setNavActive()` para sincronizar el sidebar.** Si una función como `setHistorialUser()` llama `navTo()` internamente, puede dejar el sidebar marcando una sección incorrecta. El estado activo del sidebar debe reflejar la pantalla VISIBLE al usuario, no la última navegación de fondo. 🌐
- **WWP (`#screen-app`) es la pantalla de aterrizaje por defecto para TODOS los usuarios**, incluidos admin y usuarios con `sectionPerms` de Despachos. El cambio de 2026-06-13 eliminó el ruteo que enviaba a `screen-historial` al login/restore. Despachos sigue accesible desde el sidebar pero ya no es el destino inicial. Esto aplica en web y móvil. 🌐
